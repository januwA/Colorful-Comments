...
export class Parser {
  ...
  public SetRegex(languageCode: string) {
    ...
    if (this.isPlainText && this.contributions.highlightPlainText) {
      ...
      this.expression =
        "(" +
        this.delimiters.map((e) => e.replace(/\//gi, "\\/")).join("|") +
        ")+( |\t)*";
    }
  ...
  public FindBlockComments(activeEditor: vscode.TextEditor): void {
    ...
    let regexString = "(^|[ \\t])(";
    regexString += this.blockComments[0].start;
    regexString += "[\\s])+([\\s\\S]*?)(";
    regexString += this.blockComments[0].end;
    regexString += ")";
    ...
    while ((match = regEx.exec(text))) {
      ...
      while ((line = commentRegEx.exec(commentBlock))) {
        ...
        matchTag?.ranges.push({
          range: new vscode.Range(startPos, endPos),
        });
      }
    }

    this.blockComments.shift();
  ...
  private setDelimiter(languageCode: string): void {
    ...
    switch (languageCode) {
      ...
      case "COBOL":
        this.delimiters.push(this.escapeRegExp("*>"));
        break;

      case "fortran-modern":
        this.delimiters.push("c");
        break;
  ...
  private setCommentFormat(
    ...
    if (singleLine) {
      this.delimiters.push(this.escapeRegExp(singleLine));
    } else {
      this.highlightSingleLineComments = false;
    }

    if (start && end) {
      this.blockComments.push({
        start: this.escapeRegExp(start),
        end: this.escapeRegExp(end),
      });
    }
